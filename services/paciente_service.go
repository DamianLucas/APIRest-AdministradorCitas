...
func CrearPaciente(p *models.Paciente, rol string, userID int) error {
	...
	if rol == "recepcion" && p.DoctorID == 0 {
		return utils.NewBadRequest("recepcion debe asignar un doctor al paciente")
	}

	err := repository.CrearPaciente(p)
	if err != nil {
		return utils.NewInternal("error al crear paciente")
	}
...
func ListarPacientes(rol string, userID int) ([]models.Paciente, error) {
	if rol == "doctor" {
		...
		if err != nil {
			return nil, utils.NewInternal("error al listar pacientes")
		}
...
func ObtenerPacienteID(id int, rol string, userID int) (*models.Paciente, error) {
	...
	if rol == "doctor" {
		...
		if err != nil {
			return nil, utils.NewInternal("error al validar pertenencia")
		}
		if !ok {
			return nil, utils.NewForbidden("no puede acceder a pacientes que no son suyos")
		}
	}

	paciente, err := repository.ObtenerPacienteID(id)
	if err != nil {
		return nil, utils.NewInternal("error al obtener paciente")
	}
	if paciente == nil {
		return nil, utils.NewNotFound("paciente no encontrado")
	}
	return paciente, nil
}
...
func ActualizarPaciente(id int, data *models.Paciente, rol string, userID int) error {
	...
	if rol == "doctor" {
		ok, err := repository.PacientePerteneceADoctor(id, userID)
		if err != nil {
			return utils.NewInternal("error al validar pertenencia")
		}
		if !ok {
			return utils.NewForbidden("no puede modificar pacientes que no son suyos")
		}
	}

	actualizado, err := repository.ActualizarPaciente(id, data)
	if err != nil {
		return utils.NewInternal("error al actualizar paciente")
	}
	if !actualizado {
		return utils.NewNotFound("paciente no encontrado")
	}
...
func EliminarPaciente(id int, rol string, userID int) error {
	...
	if rol == "doctor" {
		...
		if !ok {
			return utils.NewForbidden("no puede eliminar pacientes que no son suyos")
		}
	}

	eliminado, err := repository.EliminarPaciente(id)
	if err != nil {
		return utils.NewInternal("error al eliminar paciente")
	}
	if !eliminado {
		return utils.NewNotFound("paciente no encontrado")
	}
